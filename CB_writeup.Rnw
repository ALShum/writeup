...
\\
Finding available demographic variables and forming a valid GET request for ($[$VARIABLES$]$) requires a more detailed knowledge of how the census datasets are organized which variables are made available.  We discuss finding demographic variables and their format and structure in section \ref{sec:var}.
...
\end{verbatim}
This describes label 056E of concept B27001.  This is a table of 75 and older females with health insurance. \hh{Label 057E describes the corresponding number of  females aged 75 and older without health insurance. Both of these numbers are given in table \ref{tab:df}.}
The associated XML formatted version can be found the appendix in section \ref{sec:varappend}.\\
\\
In both the XML and JSON formats the different levels of factors in the labels are separated by !!.  To form a valid GET request we take the header from the JSON file (or the $id$ from the XML file) and that will be the parameter we use for ($[$VARIABLES$]$).  The concept-label ID {\tt B27001\_056E} will request the ``75 and older female with health insurance" label from the ``health insurance coverage status by sex by age" concept.  A valid HTTP GET request for this is \url{http://api.census.gov/data/[YEAR]/[DATASET]?key=[KEY]&get=B27001_056E, B27001_057E&for=[GEOGRAPHY]}.
...
\hh{Based on tables \ref{tab:df} and \ref{margin:df}}, $45052 \pm 356$ is a 90$\%$ confidence interval for the number of women in Alameda County 75 and over with health insurance.
...
\hh{We will highlight each of these issues in this section following along some of Stangl et al's examples:}

\hh{XXX this section is still a bit hard to follow. But I think, it just needs a bit of additional structure. Could you add (verbatim) the actual question that Stangl et al are asking the students to investigate? That would help for a start. Then you could switch to a two column representation with the analyses side by side (not sure that it'll work, but it's worth a try).}
...
\\
\hh{How do you actually get the data from the survey? - Mention the acs package XXX. Does that package allow a general access to the API or only to the acs data? }
...
The Census Bureau has historically provided information in the form of  counts.  However, the nominal frequency of variables may not hold much meaning as different geographic entities contain different populations.  For example, in table \ref{tab:freq} we have university enrollment by age and gender for each state from the 2012 5-year ACS dataset.  This dataset is reshaped into a tidy form.  In this dataset we have gender, age and enrollment status (public university, private university or not enrolled).  California had over 700,000 male students at public universities while Colorado only had around 80,000 students in the same category.  These nominal enrollment numbers mean very little without knowing the size of the population. \hh{XXX you might need to be a bit careful with that statement - I just found variables PE and PM in the acs variables.json file that suggest that there are now perentages and their margins included ... it doesn't mean that your point is void - it's still completely valid, but needs an extra sentence, such as  ``newest developments in the API seem to go in the direction to also include percentages and their margins, see e.g. ... ''}\\
...
\\
Due to how the data is provided by the Census Bureau compared with the subset of the public use microsample that is used in \citet{stangl:2013} there are limited combinations of variables  available.  One of the exercises in \citet{stangl:2013} is to compare health insurance status by race.  This exercise using the public use microsample is straight forward and requires a single line of code using the plyr \cite{plyr} package in R.  We can summarise percentage of people with health insurance by race as follows:
...
#print
sprintf("asian insurance_pct: %f", a_pct)
sprintf("black insurance_pct: %f", b_pct)
sprintf("other insurance_pct: %f", o_pct)
sprintf("white insurance_pct: %f", w_pct)
@
...
\hh{XXX this section still needs a bit of text - mainly motivation about how cool these examples are. Also list a couple of websites that make use of the API (you've been showing me these examples before, just find their urls again - you could even include screenshots.) }
...
ny2.df$NAME = as.numeric(as.character(ny2.df$NAME))
ny2.df$COUNTYFP = as.numeric(as.character(ny2.df$COUNTYFP))
ny3.df = merge(x = ny2.df, y = subset(fips.county, State == 'NY'), by.x = "COUNTYFP", by.y = "County.ANSI")
ny3.df$County.Name = tolower(ny3.df$County.Name)
ny3.df$County.Name = gsub(" county", "", ny3.df$County.Name)

#merge shape files with ACS data by county name
final = merge(x = ny3.df, y = f.nyc, by.x = c("NAME", "County.Name"), by.y = c("tract", "county"), all.x=TRUE)
...
qmap("brooklyn new york city", zoom=10) + geom_path(aes(x = long, y = lat, group=group, order=order), data = subset(final, COUNTYFP %in% c(61, 5, 47, 81, 85))) + geom_polygon(aes(x = long, y = lat, group = group, order = order, fill = race, alpha = 0.15), data = subset(final, COUNTYFP %in% c(61, 5, 47, 81, 85)))
@
\section{Conclusion}
\hh{XXX the first part of this section should go into the intro. (Up to about `Each of these tools ... ') Add an additional sentence here instead that summarizes the `multitude of different data products' offered by the Census Bureau .. }
