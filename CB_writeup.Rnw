...
\end{verbatim}
$[$DEVELOPER'S KEY$]$ is an id code required to perform a valid GET request.  A developer's key uniquely identifies everyone who requests data from the API.  Requesting a key can be done by registering at \url{http://www.census.gov/developers/tos/key_request.html}.\\
\\
...
\begin{verbatim}
http://api.census.gov/data/[YEAR]/[DATASET]?key=[DEVELOPER'S KEY]&get=[VARIABLES]&for=county:*&in=state:06
http://api.census.gov/data/[YEAR]/[DATASET]?key=[DEVELOPER'S KEY]&get=[VARIABLES]&for=county:037&in=state:06
\end{verbatim}
...
\begin{verbatim}
http://api.census.gov/data/[YEAR]/[DATASET]?key=[DEVELOPER'S KEY]&get=[VARIABLES]&for=tract:*&in=state:06+county037:
http://api.census.gov/data/[YEAR]/[DATASET]?key=[DEVELOPER'S KEY]&get=[VARIABLES]&for=tract:101110&in=state:06+county037:
\end{verbatim}
...
<<state, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3, fig.width=5, cache=TRUE, fig.cap='California Selected'>>=
library(ggplot2)
...
tab1 = xtable(head(dat@estimate[,1:5]), label="fig:example", caption="Health Insurance Coverage information from Online API")
digits(tab1) = rep(0,6)
print(tab1)
@
...
\\
In table \ref{fig:example} we specify state level summaries for table variable B27001.  Table variable B27001 is health insurance coverage status for various age groups grouped by gender.  Each column in table \ref{fig:example} is a sub-table on B27001.  From left-to-right, the variables in table \ref{fig:example} are: 
