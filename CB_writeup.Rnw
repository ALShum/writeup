...
\item Proportions and standard errors require a bit of calculations.
...
\begin{align*}
SE(\frac{X}{Y}) = \frac{1}{Y}\sqrt{SE(X)^2 - (\frac{X}{Y})^2 SE(B)^2}
\end{align*}
In calculating proportions we must often add different estimates together for different $labels$.  Combining the standard error for a sum of estimates: $SE(X + Y) = \sqrt{SE(X)^2 + SE(Y)^2}$.\\
%https://www.census.gov/acs/www/Downloads/data_documentation/Statistical_Testing/2012StatisticalTesting3and5.pdf
\\
Due to how the data is provided by the Census Bureau compared with the subset of the public use microsample that is used in \citet{stangl:2013} we are quite limited in the combination of variables that are available to us.
