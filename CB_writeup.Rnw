...
We examine some of the exercises listed in \citet{stangl:2013} and later discuss the difficulty in extending these exercises further by subsetting the data further.  The first example requiring calculations is exercise 3 to generate a confidence intervals for health insurance data.
...
First we calculate an estimate for the proportion of people who do not have health insurance using Stangl's dataset \citet{stangl:2013} and the corresponding 2012 5-year ACS numbers from the online API.
...
The same calculation can be done using the ACS data from the online API.  The data is not in a tidy form and require summing the correct columns together to get the desired result.  The following code prints out the first 10 column names and sums together the correct columns to find the total number of people without health insurance.
...
The corresponding 95\% confidence interval for data from the online API is calculated from the available margin of error data for each table.  Standard errors can be calculated from the margin of error using the formulas for proportion and sum from section \ref{sec:proportions}.  A 95\% confidence interval using data from the online API for the proportion of people in the united states without health insurance is: (\Sexpr{CI_L2}, \Sexpr{CI_U2}).\\
\\
From the first example, the estimates of proportion of people without health insurance is similar in both Stangl's dataset and the ACS dataset from the online API.  The standard errors are significantly different simply due to the size of Stangl's dataset compared to the size of the ACS.  In the next example we follow along with another exercise but we also try to expand further by subsetting on more factors.  Due to how data is provided by the Census Bureau compared with the dataset used by \citet{stangl:2013} there are limited combinations of variables available.  The following is another exercises from \citet{stangl:2013} to compare health insurance status by race:
