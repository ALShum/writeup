...
\hh{your solution to cut down on the length of the commands looks great!}
\end{abstract}
...
@
For the 2010 decennial census a valid HTTP GET request \hh{(see table \ref{tab:datasettable}) is formed} as follows:
\begin{verbatim}
...
$[$GEOGRAPHY$]$ describes the geographic region of interest, \hh{and the} $\&for=[GEOGRAPHY]$ tag is used to indicate this region. The geographic area can include the entire United States:
\begin{verbatim}
...
\hh{The map for Iowa's school districts can - after downloading the corresponding shapefile, e.g.~be created using the following code, which is based on R packages ggplot2 \cite{ggplot2}, maps \cite{maps}, and maptools \cite{maptools}:}
...
\label{sec:var}
\hh{XXX could you go over this section again and remove all of the duplicates? - we only need HTML or XML descriptions, and we also only need either the JSON output or a table (e.g. table 3)}
...
library(rjson)
library(plyr)
url = "http://api.census.gov/data/2012/acs5?key=b59354d11f96afaa8cbd17c5f9e5cdf7d057fc1c&get=B27001_056E,B27001_056E,NAME&for=county:*&in=state:06"
...
print(xtable(head(acsdat), label="tab:table", caption="Random subset of 2010 ACS public use microdata sample"), include.rownames=FALSE)
...
univ4 = data.frame(state = tot$state, gender = tot$gender, school = tot$school, age = tot$age, pct = tot$freq/tot$tot)
...
<<stangl2, warning=FALSE, message=FALSE, cache=TRUE>>=
library(plyr)
acsdat = read.csv("datasetSTANGL2013.csv") #Stangl's dataset
head(acsdat)
