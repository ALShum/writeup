...
\end{verbatim}
In the following examples, we will examine various geographies available from the online API and their corresponding HTTP GET requests.  We will use the United States as a whole and various geographic areas within the state of California for our examples.\\
...
qplot(long, lat, data=counties.map, group=group, order=order, geom="polygon", fill=factor(CA)) + 
  theme(legend.position = "none")
@
Below states we can select counties and census tracts.  At the county level, after we specify California we can look at counties within California (summary level 050).  We can see from figure \ref{fig:county} that we have selected Los Angeles county within California state.  For a corresponding HTTP GET request the $[$GEOGRAPHY$]$ is \textit{county:037$\&$in=state:06}.  There are a few other geographic regions we can subset by within a state such as ZIP code tabulation area; we could have selected 90210 which corresponds to Beverly Hills, California.  The required $[$GEOGRAPHY$]$ for selecting 90210 is \textit{zip+code+tabulation+area:90210}.\\
