...
The paper by \citet{stangl:2013} discusses the use of the American Community Survey as a dataset for classroom exercises (at the undergraduate level) with an emphasis on how data collected from these surveys is used to make important policy decisions.  The paper contains a number of classroom exercises that ask the reader to calculate basic proportions about various demographic data such as health insurance coverage, marriage status and income.  One of the main goals of these exercises to to engage students with a real example and according to \citet{edu1} using a real dataset is a good way for students to think about data and statistical issues.  The particular dataset used by \citet{stangl:2013} is a 1000 observation random subset of the 2010 ACS public use microdata sample which constitutes a 0.005\% subsample of the 2010 ACS, or about a 0.0003\% representation of the US population. It would be advantageous by far to get corresponding answers based on the whole of the ACS 5-year dataset.  We will attempt to use the Census Bureau online API to examine the same demographic variables.\\
\\
...
We examine some of the exercises listed in \citet{stangl:2013} and later discuss the difficulty in extending these exercises by subsetting on additional factors.  The first example is exercise 3 from \citet{stangl:2013} and the exercise is to generate confidence intervals for health insurance data.  The exercise is listed below:
...
print(tabl, include.rownames=FALSE)
@
From this data we can generate a similar histogram with one observation per state, this histogram is in figure \ref{fig:example3}.  However, if instead we are interested in median income for full-term works average over both genders, this data table would not be entirely useful.  From table \ref{tab:medincome}, it is clear that it is not possible to calculate the exact median income for full-time works averaged over both genders without more information.  In the previous example the main limitation was not being able to subset on additional variables, in this example the main limitation is not being able to average over the available variables.
